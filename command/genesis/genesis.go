...
func setFlags(cmd *cobra.Command) {
	cmd.Flags().StringVar(
		...
		"the directory for the Favo Edge genesis data",
	)
...
func setLegacyFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Uint64Var(
		...
		chainIDFlagLEGACY,
		command.DefaultChainID,
		"the ID of the chain (not-applicable for Favobft consensus protocol as chain id is defined in manifest.json)",
	)
...
func runCommand(cmd *cobra.Command, _ []string) {
	...
	if params.isFavoBFTConsensus() {
		err = params.generateFavoBftChainConfig(outputter)
	} else {
