...
import (
	...
	"github.com/newton2049/favo-chain/server"
...
// generateFavoBftChainConfig creates and persists favobft chain configuration to the provided file path
func (p *genesisParams) generateFavoBftChainConfig(o command.OutputFormatter) error {
	// load manifest file
	manifest, err := favobft.LoadManifest(p.manifestPath)
	if err != nil {
	...
	var bridge *favobft.BridgeConfig
	...
	favoBftConfig := &favobft.FavoBFTConfig{
	...
	chainConfig := &chain.Chain{
		...
		Params: &chain.Params{
			...
			Engine: map[string]interface{}{
				string(server.FavoBFTConsensus): favoBftConfig,
			},
		},
	...
	validatorMetadata := make([]*favobft.ValidatorMetadata, len(manifest.GenesisValidators))
	...
	genesisExtraData, err := generateExtraDataFavoBft(validatorMetadata)
	if err != nil {
		return err
	}
	...
	chainConfig.Genesis = &chain.Genesis{
		...
		Mixhash:    favobft.FavoBFTMixDigest,
	}
...
// generateExtraDataFavoBft populates Extra with specific fields required for favobft consensus protocol
func generateExtraDataFavoBft(validators []*favobft.ValidatorMetadata) ([]byte, error) {
	delta := &favobft.ValidatorSetDelta{
		Added:   validators,
		Removed: bitmap.Bitmap{},
	}

	extra := favobft.Extra{Validators: delta, Checkpoint: &favobft.CheckpointData{}}

	return append(make([]byte, favobft.ExtraVanity), extra.MarshalRLPTo(nil)...), nil
}
