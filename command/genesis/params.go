...
func (p *genesisParams) validateFlags() error {
	...
	if p.epochSize < 2 && (p.isIBFTConsensus() || p.isFavoBFTConsensus()) {
...
func (p *genesisParams) isFavoBFTConsensus() bool {
	return server.ConsensusType(p.consensusRaw) == server.FavoBFTConsensus
}
...
func (p *genesisParams) initRawParams() error {
	...
	if p.consensus == server.FavoBFTConsensus {
		return nil
	}
