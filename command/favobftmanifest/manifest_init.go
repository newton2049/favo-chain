...
func GetCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "manifest",
		Short:   "Initializes manifest file. It is applicable only to favobft consensus protocol.",
		PreRunE: runPreRun,
		Run:     runCommand,
	}
...
func setFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(
		...
		validatorsPathFlag,
		"./",
		"root path containing favobft validator keys",
	)
	...
	cmd.Flags().StringVar(
		...
		"folder prefix names for favobft validator keys",
	)
...
func runCommand(cmd *cobra.Command, _ []string) {
	...
	manifest := &favobft.Manifest{GenesisValidators: validators, ChainID: params.chainID}
...
func (p *manifestInitParams) getValidatorAccounts() ([]*favobft.Validator, error) {
	...
	if len(p.validators) > 0 {
		validators := make([]*favobft.Validator, len(p.validators))
		for i, validator := range p.validators {
			...
			validators[i] = &favobft.Validator{
				MultiAddr:    parts[0],
				Address:      addr,
