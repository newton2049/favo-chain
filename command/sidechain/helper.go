...
func ValidateSecretFlags(dataDir, config string) error {
	if config == "" {
		if dataDir == "" {
			return favobftsecrets.ErrInvalidParams
		} else {
...
func GetAccount(accountDir, accountConfig string) (*wallet.Account, error) {
	...
	secretsManager, err := favobftsecrets.GetSecretsManager(accountDir, accountConfig, true)
	if err != nil {
		return nil, err
	}
...
func GetValidatorInfo(validatorAddr ethgo.Address, txRelayer txrelayer.TxRelayer) (*favobft.ValidatorInfo, error) {
