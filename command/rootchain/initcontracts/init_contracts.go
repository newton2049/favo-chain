...
import (
	...
	"github.com/newton2049/favo-chain/command/rootchain/helper"
	"github.com/newton2049/favo-chain/consensus/favobft"
	"github.com/newton2049/favo-chain/consensus/favobft/contractsapi"
	"github.com/newton2049/favo-chain/consensus/favobft/contractsapi/artifact"
...
var (
	...
	metadataPopulatorMap = map[string]func(*favobft.RootchainConfig, types.Address){
		stateSenderName: func(rootchainConfig *favobft.RootchainConfig, addr types.Address) {
			rootchainConfig.StateSenderAddress = addr
		},
		checkpointManagerName: func(rootchainConfig *favobft.RootchainConfig, addr types.Address) {
			rootchainConfig.CheckpointManagerAddress = addr
		},
		blsName: func(rootchainConfig *favobft.RootchainConfig, addr types.Address) {
			rootchainConfig.BLSAddress = addr
		},
		bn256G2Name: func(rootchainConfig *favobft.RootchainConfig, addr types.Address) {
			rootchainConfig.BN256G2Address = addr
		},
		exitHelperName: func(rootchainConfig *favobft.RootchainConfig, addr types.Address) {
			rootchainConfig.ExitHelperAddress = addr
		},
		rootERC20PredicateName: func(rootchainConfig *favobft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootERC20PredicateAddress = addr
		},
		rootERC20Name: func(rootchainConfig *favobft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootNativeERC20Address = addr
		},
		erc20TemplateName: func(rootchainConfig *favobft.RootchainConfig, addr types.Address) {
...
func runCommand(cmd *cobra.Command, _ []string) {
	...
	manifest, err := favobft.LoadManifest(params.manifestPath)
	if err != nil {
...
func deployContracts(outputter command.OutputFormatter, client *jsonrpc.Client,
	manifest *favobft.Manifest) error {
	...
	rootchainConfig := &favobft.RootchainConfig{}
...
func initializeExitHelper(txRelayer txrelayer.TxRelayer, rootchainConfig *favobft.RootchainConfig,
	deployerKey ethgo.Key) error {
...
func initializeRootERC20Predicate(txRelayer txrelayer.TxRelayer, rootchainConfig *favobft.RootchainConfig,
	deployerKey ethgo.Key) error {
...
func validatorSetToABISlice(o command.OutputFormatter,
	validators []*favobft.Validator) ([]*contractsapi.Validator, error) {
	accSet := make(favobft.AccountSet, len(validators))
	...
	for i, validator := range validators {
		...
		accSet[i] = &favobft.ValidatorMetadata{
