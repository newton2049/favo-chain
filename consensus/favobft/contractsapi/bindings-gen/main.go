...
import (
	...
	gensc "github.com/newton2049/favo-chain/consensus/favobft/contractsapi"
...
`
	...
	if err := ioutil.WriteFile("./consensus/favobft/contractsapi/contractsapi.go", output, 0600); err != nil {
		log.Fatal(err)
	}
}
...
func generateType(generatedData *generatedData, name string, obj *abi.Type, res *[]string) (string, error) {
	...
	for _, tupleElem := range obj.TupleElems() {
		...
		// uint8 internally in favobft, we can use regexp to replace those values with the
