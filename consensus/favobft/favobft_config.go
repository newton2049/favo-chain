...
// FavoBFTConfig is the configuration file for the Favobft consensus protocol.
type FavoBFTConfig struct {
...
// GetFavoBFTConfig deserializes provided chain config and returns FavoBFTConfig
func GetFavoBFTConfig(chainConfig *chain.Chain) (FavoBFTConfig, error) {
	consensusConfigJSON, err := json.Marshal(chainConfig.Params.Engine["favobft"])
	if err != nil {
		return FavoBFTConfig{}, err
	}

	var favoBFTConfig FavoBFTConfig
	err = json.Unmarshal(consensusConfigJSON, &favoBFTConfig)

	if err != nil {
		return FavoBFTConfig{}, err
	}

	return favoBFTConfig, nil
}
...
func (p *FavoBFTConfig) IsBridgeEnabled() bool {
	return p.Bridge != nil
}
