...
import (
	...
	"github.com/newton2049/favo-chain/consensus/ibft/signer"
	"github.com/newton2049/favo-chain/helper/progress"
	"github.com/newton2049/favo-chain/txpool"
...
// the test initializes favobft and chain mock (map of headers) after which a new header is verified
...
func TestFavobft_VerifyHeader(t *testing.T) {
	t.Parallel()
	...
	for i := uint64(1); i < favoBftConfig.EpochSize; i++ {
	...
	// create favobft with appropriate mocks
	favobft := &Favobft{
	...
	parentHeader := &types.Header{
		Number:    favoBftConfig.EpochSize,
	...
	currentHeader := &types.Header{
		Number:     favoBftConfig.EpochSize + 1,
		ParentHash: parentHeader.Hash,
		Timestamp:  parentHeader.Timestamp + 1,
		MixHash:    FavoBFTMixDigest,
		Difficulty: 1,
	}
	...
	assert.ErrorContains(t, favobft.VerifyHeader(currentHeader), "invalid header hash")
	...
	assert.ErrorContains(t, favobft.VerifyHeader(currentHeader), "failed to verify signatures for parent of block")
	...
	assert.NoError(t, favobft.VerifyHeader(currentHeader))
	...
	favobft.validatorsCache = newValidatorsSnapshotCache(hclog.NewNullLogger(), newTestState(t), blockchainMock)
	assert.NoError(t, favobft.validatorsCache.storeSnapshot(&validatorSnapshot{Epoch: 0, Snapshot: validatorSetCurrent})) // invalid validator set is submitted
	assert.NoError(t, favobft.validatorsCache.storeSnapshot(&validatorSnapshot{Epoch: 1, Snapshot: validatorSetCurrent}))
	assert.ErrorContains(t, favobft.VerifyHeader(currentHeader), "failed to verify signatures for parent of block")

	// clean validators cache again and set valid snapshots
	favobft.validatorsCache = newValidatorsSnapshotCache(hclog.NewNullLogger(), newTestState(t), blockchainMock)
	assert.NoError(t, favobft.validatorsCache.storeSnapshot(&validatorSnapshot{Epoch: 0, Snapshot: validatorSetParent}))
	assert.NoError(t, favobft.validatorsCache.storeSnapshot(&validatorSnapshot{Epoch: 1, Snapshot: validatorSetCurrent}))
	assert.NoError(t, favobft.VerifyHeader(currentHeader))
...
func TestFavobft_Close(t *testing.T) {
	t.Parallel()
	...
	assert.NoError(t, favobft.Close())
	...
	favobft.closeCh = make(chan struct{})

	assert.Error(t, errExpected, favobft.Close())

	select {
	case <-favobft.closeCh:
...
func TestFavobft_GetSyncProgression(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, result, favobft.GetSyncProgression())
}
...
func Test_Factory(t *testing.T) {
	...
	favobft, ok := r.(*Favobft)
	require.True(t, ok)

	assert.Equal(t, txPool, favobft.txPool)
	assert.Equal(t, epochSize, favobft.consensusConfig.EpochSize)
	assert.Equal(t, params, favobft.config)
}
