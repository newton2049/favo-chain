...
import (
	...
	secretsHelper "github.com/newton2049/favo-chain/secrets/helper"
...
func TestIntegration_CommitEpoch(t *testing.T) {
	...
	for _, currentValidators := range validatorSets {
		...
		// validator data for favobft config
		...
		for i, validator := range accSet {
			...
			// create validator data for favobft config
		...
		initInput, err := getInitChildValidatorSetInput(favoBFTConfig)
		require.NoError(t, err)
		...
		commitEpoch := createTestCommitEpochInput(t, 1, accSet, favoBFTConfig.EpochSize)
		...
		commitEpoch = createTestCommitEpochInput(t, 2, accSet, favoBFTConfig.EpochSize)
		input, err = commitEpoch.EncodeAbi()
		require.NoError(t, err)
