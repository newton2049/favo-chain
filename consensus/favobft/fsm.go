...
type fsm struct {
	// FavoBFT consensus protocol configuration
	...
	// favobftBackend implements methods needed from the favobft
...
func (f *fsm) Validate(proposal []byte) error {
	...
	if err := extra.ValidateParentSignatures(block.Number(), f.favobftBackend, nil, f.parent, parentExtra,
	...
	if f.logger.IsTrace() && block.Number() > 1 {
		validators, err := f.favobftBackend.GetValidators(block.Number()-2, nil)
		if err != nil {
...
func validateHeaderFields(parent *types.Header, header *types.Header) error {
	...
	// verify mix digest
	if header.MixHash != FavoBFTMixDigest {
