...
import (
	...
	"github.com/newton2049/favo-chain/consensus/favobft/bitmap"
...
func TestConsensusRuntime_isFixedSizeOfEpochMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.FavoBFTConfig.EpochSize = c.epochSize
...
func TestConsensusRuntime_isFixedSizeOfSprintMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.FavoBFTConfig.SprintSize = c.sprintSize
		runtime.epoch.FirstBlockInEpoch = c.firstBlockInEpoch
		assert.False(t,
...
func TestConsensusRuntime_OnBlockInserted_EndOfEpoch(t *testing.T) {
	...
	favobftBackendMock := new(favobftBackendMock)
	favobftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validatorSet).Times(3)
	...
	config := &runtimeConfig{
		...
		favobftBackend: favobftBackendMock,
		txPool:         txPool,
...
func TestConsensusRuntime_OnBlockInserted_MiddleOfEpoch(t *testing.T) {
	...
	favobftBackendMock := new(favobftBackendMock)
	favobftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(nil).Once()
	...
	config := &runtimeConfig{
		FavoBFTConfig: &FavoBFTConfig{EpochSize: epochSize},
		blockchain:    blockchainMock,
		txPool:        txPool,
	}
...
func Test_NewConsensusRuntime(t *testing.T) {
	...
	favobftBackendMock := new(favobftBackendMock)
	favobftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators).Twice()
	...
	assert.Equal(t, uint64(10), runtime.config.FavoBFTConfig.SprintSize)
	assert.Equal(t, uint64(10), runtime.config.FavoBFTConfig.EpochSize)
	assert.Equal(t, "0x0000000000000000000000000000000000000101", contracts.ValidatorSetContract.String())
	assert.Equal(t, "0x1300000000000000000000000000000000000000", runtime.config.FavoBFTConfig.Bridge.BridgeAddr.String())
	assert.Equal(t, "0x1000000000000000000000000000000000000000", runtime.config.FavoBFTConfig.Bridge.CheckpointAddr.String())
	assert.True(t, runtime.IsBridgeEnabled())
	systemStateMock.AssertExpectations(t)
	blockchainMock.AssertExpectations(t)
	favobftBackendMock.AssertExpectations(t)
}
...
func TestConsensusRuntime_calculateCommitEpochInput_SecondEpoch(t *testing.T) {
	...
	favobftBackendMock := new(favobftBackendMock)
	favobftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators.getPublicIdentities()).Twice()
...
func TestConsensusRuntime_HasQuorum(t *testing.T) {
	...
	config := &runtimeConfig{
		...
		blockchain:    blockchainMock,
		FavoBFTConfig: &FavoBFTConfig{EpochSize: 10, SprintSize: 5},
	}
	runtime := &consensusRuntime{
		state:          state,
		config:         config,
