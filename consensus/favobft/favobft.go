// Package favobft implements PBFT consensus algorithm integration and bridge feature
package favobft
...
import (
	...
	"github.com/newton2049/favo-chain/consensus/favobft/wallet"
	"github.com/newton2049/favo-chain/contracts"
	"github.com/newton2049/favo-chain/helper/common"
	"github.com/newton2049/favo-chain/helper/progress"
	"github.com/newton2049/favo-chain/network"
	"github.com/newton2049/favo-chain/state"
	"github.com/newton2049/favo-chain/syncer"
...
// favobftBackend is an interface defining favobft methods needed by fsm and sync tracker
type favobftBackend interface {
...
func Factory(params *consensus.Params) (consensus.Consensus, error) {
	logger := params.Logger.Named("favobft")
	...
	// initialize favobft consensus config
	customConfigJSON, err := json.Marshal(params.Config.Config)
	if err != nil {
		return nil, err
	}

	err = json.Unmarshal(customConfigJSON, &favobft.consensusConfig)
	if err != nil {
		return nil, err
	}
...
type Favobft struct {
	...
	// consensusConfig is genesis configuration for favobft consensus protocol
	consensusConfig *FavoBFTConfig
...
func GenesisPostHookFactory(config *chain.Chain, engineName string) func(txn *state.Transition) error {
	return func(transition *state.Transition) error {
		favoBFTConfig, err := GetFavoBFTConfig(config)
		if err != nil {
			return err
		}

		// initialize ChildValidatorSet SC
		input, err := getInitChildValidatorSetInput(favoBFTConfig)
		if err != nil {
			return err
		}
		...
		input, err = getInitChildERC20PredicateInput(favoBFTConfig.Bridge)
		if err != nil {
			return err
		}
		...
		if favoBFTConfig.Bridge != nil {
			rootNativeERC20Token = favoBFTConfig.Bridge.RootNativeERC20Addr
		}

		if favoBFTConfig.MintableERC20Token {
			...
			params := &contractsapi.InitializeNativeERC20MintableFn{
				...
				Owner_:     favoBFTConfig.Governance,
...
func (p *Favobft) Initialize() error {
	p.logger.Info("initializing favobft...")
	...
	// initialize favobft consensus data directory
	p.dataDir = filepath.Join(p.config.Config.Path, "favobft")
...
func (p *Favobft) Start() error {
	p.logger.Info("starting favobft consensus", "signer", p.key.String())
...
func (p *Favobft) initRuntime() error {
	runtimeConfig := &runtimeConfig{
		FavoBFTConfig:         p.consensusConfig,
		Key:                   p.key,
		DataDir:               p.dataDir,
		State:                 p.state,
...
func (p *Favobft) startRuntime() error {
	go p.startConsensusProtocol()

	return nil
}

func (p *Favobft) startConsensusProtocol() {
...
func (p *Favobft) waitForNPeers() bool {
	for {
		select {
		case <-p.closeCh:
			return false
...
func (p *Favobft) GetSyncProgression() *progress.Progression {
...
func (p *Favobft) VerifyHeader(header *types.Header) error {
...
func (p *Favobft) verifyHeaderImpl(parent, header *types.Header, parents []*types.Header) error {
...
func (p *Favobft) GetValidators(blockNumber uint64, parents []*types.Header) (AccountSet, error) {
...
func (p *Favobft) ProcessHeaders(_ []*types.Header) error {
	// Not required
	return nil
}

// GetBlockCreator retrieves the block creator (or signer) given the block header
func (p *Favobft) GetBlockCreator(h *types.Header) (types.Address, error) {
	return types.BytesToAddress(h.Miner), nil
}

// PreCommitState a hook to be called before finalizing state transition on inserting block
func (p *Favobft) PreCommitState(_ *types.Header, _ *state.Transition) error {
	// Not required
	return nil
}

// GetBridgeProvider returns an instance of BridgeDataProvider
func (p *Favobft) GetBridgeProvider() consensus.BridgeDataProvider {
	return p.runtime
}
