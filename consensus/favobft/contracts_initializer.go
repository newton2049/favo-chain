...
var (
	nativeTokenName     = "FAVO"
	nativeTokenSymbol   = "FAVO"
...
func getInitChildValidatorSetInput(favoBFTConfig FavoBFTConfig) ([]byte, error) {
	apiValidators := make([]*contractsapi.ValidatorInit, len(favoBFTConfig.InitialValidatorSet))

	for i, validator := range favoBFTConfig.InitialValidatorSet {
	...
	params := &contractsapi.InitializeChildValidatorSetFn{
		Init: &contractsapi.InitStruct{
			EpochReward:   new(big.Int).SetUint64(favoBFTConfig.EpochReward),
			MinStake:      big.NewInt(minStake),
			MinDelegation: big.NewInt(minDelegation),
			EpochSize:     new(big.Int).SetUint64(favoBFTConfig.EpochSize),
		},
		NewBls:     contracts.BLSContract,
		Governance: favoBFTConfig.Governance,
