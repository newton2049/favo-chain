...
build:
	...
	go build -o favo-chain -ldflags="\
    	-X 'github.com/newton2049/favo-chain/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/newton2049/favo-chain/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/newton2049/favo-chain/versioning.Branch=$(BRANCH)'\
		-X 'github.com/newton2049/favo-chain/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/favo-chain .
	env EDGE_BINARY=${PWD}/artifacts/favo-chain go test -v -timeout=30m ./e2e/...

.PHONY: test-e2e-favobft
test-e2e-favobft:
    # We can not build with race because of a bug in boltdb dependency
	go build -o artifacts/favo-chain .
	env EDGE_BINARY=${PWD}/artifacts/favo-chain E2E_TESTS=true E2E_LOGS=true E2E_TESTS_TYPE=integration \
	go test -v -timeout=45m ./e2e-favobft/e2e/...

test-property-favobft:
    # We can not build with race because of a bug in boltdb dependency
	go build -o artifacts/favo-chain .
	env EDGE_BINARY=${PWD}/artifacts/favo-chain E2E_TESTS=true E2E_LOGS=true E2E_TESTS_TYPE=property go test -v -timeout=30m ./e2e-favobft/property/...
...
generate-smart-contract-bindings:
	go run ./consensus/favobft/contractsapi/artifacts-gen/main.go
	go run ./consensus/favobft/contractsapi/bindings-gen/main.go
