...
import (
	...
	"github.com/newton2049/favo-chain/blockchain/storage"
	"github.com/newton2049/favo-chain/blockchain/storage/leveldb"
	"github.com/newton2049/favo-chain/blockchain/storage/memory"
	...
	"github.com/newton2049/favo-chain/consensus/favobft/statesyncrelayer"
	...
	configHelper "github.com/newton2049/favo-chain/helper/config"
	"github.com/newton2049/favo-chain/helper/progress"
	"github.com/newton2049/favo-chain/jsonrpc"
	...
	"github.com/newton2049/favo-chain/state/runtime"
	"github.com/newton2049/favo-chain/state/runtime/allowlist"
	"github.com/newton2049/favo-chain/state/runtime/tracer"
	"github.com/newton2049/favo-chain/txpool"
	"github.com/newton2049/favo-chain/types"
	"github.com/newton2049/favo-chain/validate"
...
type Server struct {
	...
	// stateSyncRelayer is handling state syncs execution (Favobft exclusive)
...
func NewServer(config *Config) (*Server, error) {
	...
	if ConsensusType(engineName) == FavoBFTConsensus {
		favoBFTConfig, err := consensusFavoBFT.GetFavoBFTConfig(config.Chain)
		if err != nil {
			return nil, err
		}

		if favoBFTConfig.InitialTrieRoot != types.ZeroHash {
			checkedInitialTrieRoot, err := itrie.HashChecker(favoBFTConfig.InitialTrieRoot.Bytes(), stateStorage)
			if err != nil {
				return nil, fmt.Errorf("error on state root verification %w", err)
			}

			if checkedInitialTrieRoot != favoBFTConfig.InitialTrieRoot {
				return nil, errors.New("invalid initial state root")
			}

			logger.Info("Initial state root checked and correct")

			initialStateRoot = favoBFTConfig.InitialTrieRoot
		}
	}
...
func (s *Server) setupRelayer() error {
	...
	favoBFTConfig, err := consensusFavoBFT.GetFavoBFTConfig(s.config.Chain)
	if err != nil {
		return fmt.Errorf("failed to extract favobft config: %w", err)
	}

	trackerStartBlockConfig := map[types.Address]uint64{}
	if favoBFTConfig.Bridge != nil {
		trackerStartBlockConfig = favoBFTConfig.Bridge.EventTrackerStartBlocks
	}
