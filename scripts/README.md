...
## Local development
Running `favo-chain` from local binary can be done very easily by using provided `scripts` folder.

* `scripts/cluster ibft` - deploy environment with `ibft` consensus
* `scripts/cluster favobft` - deploy environment with `favobft` consensus
