...
func (t *TestServer) Start() {
	...
	args := []string{
		"server",
		// add data dir
		"--" + favobftsecrets.AccountDirFlag, config.DataDir,
		// add custom chain
		"--chain", config.Chain,
		// enable p2p port
...
func (t *TestServer) Stake(amount uint64) error {
	args := []string{
		"favobft",
		"stake",
		"--" + favobftsecrets.AccountDirFlag, t.config.DataDir,
...
func (t *TestServer) RegisterValidator(secrets string, stake string) error {
	args := []string{
		"favobft",
		"register-validator",
		"--" + favobftsecrets.AccountDirFlag, path.Join(t.clusterConfig.TmpDir, secrets),
		"--jsonrpc", t.JSONRPCAddr(),
	}
...
func (t *TestServer) Delegate(amount uint64, secrets string, validatorAddr ethgo.Address) error {
	args := []string{
		"favobft",
		"stake",
		"--" + favobftsecrets.AccountDirFlag, secrets,
		"--jsonrpc", t.JSONRPCAddr(),
...
func (t *TestServer) HasValidatorSealed(firstBlock, lastBlock uint64, validators favobft.AccountSet,
	...
	for i := firstBlock + 1; i <= lastBlock; i++ {
		...
		extra, err := favobft.GetIbftExtra(block.ExtraData)
		if err != nil {
			return false, err
		}
