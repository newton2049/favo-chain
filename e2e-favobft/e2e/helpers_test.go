...
func getRootchainValidators(relayer txrelayer.TxRelayer, checkpointManagerAddr ethgo.Address) ([]*favobft.ValidatorInfo, error) {
	...
	validators := make([]*favobft.ValidatorInfo, validatorsCount)
	...
	for i := 0; i < int(validatorsCount); i++ {
		...
		validators[i] = &favobft.ValidatorInfo{
...
func sendExitTransaction(
	...
	proofExitEventEncoded, err := exitEventAPI.Encode(&favobft.ExitEvent{
		ID:       exitEventID,
