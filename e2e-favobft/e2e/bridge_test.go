...
import (
	...
	rootchainHelper "github.com/newton2049/favo-chain/command/rootchain/helper"
	"github.com/newton2049/favo-chain/command/sidechain"
	...
	"github.com/newton2049/favo-chain/e2e-favobft/framework"
...
func TestE2E_Bridge_DepositAndWithdrawERC20(t *testing.T) {
	...
	manifest, err := favobft.LoadManifest(path.Join(cluster.Config.TmpDir, manifestFileName))
	require.NoError(t, err)
	...
	currentExtra, err := favobft.GetIbftExtra(currentBlock.ExtraData)
	require.NoError(t, err)
...
func TestE2E_Bridge_ChangeVotingPower(t *testing.T) {
	...
	queryValidators := func(handler func(idx int, validatorInfo *favobft.ValidatorInfo)) {
	...
	originalValidatorStorage := make(map[ethgo.Address]*favobft.ValidatorInfo, votingPowerChanges)

	queryValidators(func(idx int, validator *favobft.ValidatorInfo) {
